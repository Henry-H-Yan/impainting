function [inpaintedImg,origImg,fillImg,C,D,fillMovie] = newInpaint2(img,  map,  fillColor)

% 
% function [inpaintedImg,origImg,fillImg,C,D,fillMovie] = newInpaint2(imgFilename,mapFilename,fillColor)
...
map3D=cat(3, zeros(sz), map*255, zeros(sz));
for y=1:sz(1)
    for x=1:sz(2)
        if map(y,x)==1
            fillImg(y,x,:)=map3D(y,x,:);
        end
    end
end
